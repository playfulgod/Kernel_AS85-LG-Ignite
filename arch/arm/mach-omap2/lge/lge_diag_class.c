...
//extern char process_inputval[128]; 
...
char process_inputval[128];
EXPORT_SYMBOL(process_inputval);
...
{
	printk("\n%s:%s\n", __func__,buf);
	memset(process_value,0,sizeof(process_value));
...
static ssize_t inputval_show(struct device *dev, struct device_attribute *attr,	char *buf)
{
	printk("\n%s:%s\n", __func__,process_inputval);
	memcpy(buf,process_inputval,sizeof(process_inputval));
	return sizeof(process_inputval);

}

static ssize_t inputval_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	printk("\n%s:%s\n", __func__,buf);
	memset(process_inputval,0,sizeof(process_inputval));
	memcpy(process_inputval,buf,count);
	//	strcpy(process_inputval, buf);
	return count;
}
...
static ssize_t fota_id_check_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", fota_id_check);
}

static ssize_t fota_id_check_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	sscanf(buf, "%d", &fota_id_check);
...
static DEVICE_ATTR(inputval, S_IRUGO | S_IWUSR | S_IWGRP | S_IWOTH, inputval_show, inputval_store);

// LGE_CHANGE_E [myeonggyu.son@lge.com] [2011.02.21] [gelato] add diag sysfs [END]

static DEVICE_ATTR(fota_id_check, S_IRUGO | S_IWUSR | S_IWGRP | S_IWOTH, fota_id_check_show, fota_id_check_store);
...
ret = device_create_file(sdev->dev, &dev_attr_inputval);
if (ret < 0)
	goto err_create_file_9;


	ret = device_create_file(sdev->dev, &dev_attr_fota_id_check);
	if (ret < 0)
		goto err_create_file_10;
...
err_create_file_10:
	device_remove_file(sdev->dev, &dev_attr_fota_id_check);
	
err_create_file_9:
	device_remove_file(sdev->dev, &dev_attr_inputval);
