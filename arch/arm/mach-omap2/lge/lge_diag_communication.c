...
// 20110611 jihang.chung@lge.com test script on -> go to Idle END
				...
				case TEST_MODE_FOTA_ID_CHECK:
					update_diagcmd_state(diagpdev,"FOTAIDCHECK", ctrl_buf.state);
					break;
					
//20110831 yongman.kwon@lge.com [LS855] block slate [START]
#if 0						
				/* for slate */	
				...
				case ICD_SETSCREENORIENTATIONLOCK_REQ_IOCTL:
					...
					break;
#endif
//20110831 yongman.kwon@lge.com [LS855] block slate [END]

			     case APNV_READ_REQ_IOCTL:
					update_diagcmd_state(diagpdev,"APNV_READ", ctrl_buf.state); 
					break;
					
				case APNV_WRITE_REQ_IOCTL:
				   update_diagcmd_state(diagpdev,"APNV_WRITE", ctrl_buf.state); 
				   break;
