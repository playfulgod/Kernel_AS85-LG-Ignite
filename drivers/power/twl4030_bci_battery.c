...
#define EOC_VOLTAGE_LIMIT 4190

// NO Battery adc 634 
//20110721 yongman.kwon@lge.com [LS855] change no battery adc by HW.
//#define NO_BATTERY_ADC_VALUE 620
#define NO_BATTERY_ADC_VALUE 590
#define BAD_BATTERY_TEMP  -1000
...
static int chg_complete_state = 0;	//20101104 taehwan.kim@lge.com to fix trickle chg
#endif
// 20100624 taehwan.kim@lge.com  To add Hub battery support[END_LGE]
...
int set_end_of_charge(int complete)
{
	...
	if(complete) {
		...
		if( (refer_di->battery_capacity != 100) 
				|| (refer_di->voltage_uV < EOC_VOLTAGE_LIMIT) ) {
			printk("[charging_msg] %s: ******** EOC occured too early...volt %d, Cap %d...\n", __func__, refer_di->voltage_uV, refer_di->battery_capacity);
		} else {
			printk("[charging_msg] %s: ******** EOC occured at volt %d, Cap %d\n", __func__, refer_di->voltage_uV, refer_di->battery_capacity);
		}
...
static int adc2temperature(int adc)
{
	...
	static struct temperature_table thm_tbl[] = { // XXX_mbk tunning
#if 0 /* P970 */
		{ 600,	120},	//  60 C
		{ 500,	155},	//  50 C
		{ 400,	205},	//  40 C
...
#if 1
	//printk("%s: batt therm adc %d\n", __func__, temp);
	if( -1 == prev_temp ) {
		prev_temp = temp;
	} else {
		/* maximum value whthin 3sec - 5C*/
		if (30 < (temp - prev_temp))
		{
			temp = prev_temp + 30;
		}
		else if (30 < (prev_temp - temp))
		{
			temp = prev_temp - 30;
		}
	}
	//printk("%s: tunning 1  adc %d\n", __func__, temp);
	temp = (prev_temp*4 + temp) / 5;
	//printk("%s: average    adc %d\n", __func__, temp);
	prev_temp = temp;
#endif
...
#ifdef CONFIG_LGE_BATT_THERM_LAB3_SCENARIO
struct batt_therm_tunning_data_type {
	int batt_only_warning_temp;
	int batt_only_pwroff_temp;
	int with_pwr_current_limit_temp;
	int with_pwr_stop_charging_temp;
	int with_pwr_stop_charging_low_temp;
	int with_pwr_stop_charging_volt;
	int with_pwr_to_normal_temp;
	int with_pwr_to_normal_low_temp;
};

static struct batt_therm_tunning_data_type batt_therm_data = {
	570, 	/* batt_only_warning_temp*/
	600, 	/* batt_only_pwroff_temp*/
	450, 	/* with_pwr_current_limit_temp*/
	550, 	/* with_pwr_stop_charging_temp*/
	-100, 	/* with_pwr_stop_charging_low_temp*/
	4000, 	/* with_pwr_stop_charging_volt*/
	420, 	/* with_pwr_to_normal_temp - 420 */
	-50, 	/* with_pwr_to_normal_low_temp - 50 */
};

enum{
	BATT_THRM_NORMAL,
	BATT_THRM_WARNING_BATT_ONLY, 			/* 57 ~ 60 */
	BATT_THRM_PWROFF, 						/* over 60 */
	BATT_THRM_CURRENT_LIMIT, 				/* over 45 & under 4V */
	BATT_THRM_STOP_CHARGING,				/* ( under -10 ) || ( over 45 & over 4V ) */
	BATT_THRM_WARNING_AND_STOP_CHARGING, 	/* over 55 */
};

static char *batt_thrm_status_text[] = {
	"Normal", "Warning_batt_only", "poweroff", "current limit", "stop charging", "warning and stop charging"
};


static int batt_thrm_mass_product_cable(void)
{
	int ret_val = 0;
...
int batt_thrm_state_fake_mode = 0;
static int batt_thrm_state_working_condition(struct twl4030_bci_device_info *di)
{
	// Battery Thermistor 가 동작 안하는경우
	//  0. battery thrm state fake mode시에..
	//  1. Battery Fake Mode시에.....
	//  2. Battery가 없는 경우.. 
	//  3. LT인 경우..
	//  4. Battery ADC가 불량인경우..
	if ( batt_thrm_state_fake_mode )
		return 0;
	...
	if( batt_thrm_mass_product_cable() )
		return 0;

	if( BAD_BATTERY_TEMP == di->temp_C )
		return 0;

	return 1;
}

static void batt_thrm_state_func(struct twl4030_bci_device_info *di)
{
	if( !batt_thrm_state_working_condition(di))
		return ;
...
#endif
#else
	if ( POWER_SUPPLY_TYPE_BATTERY != di->charge_rsoc )
#endif
	{
		/* with TA and USB */
		if(di->temp_C > batt_therm_data.with_pwr_stop_charging_temp ) {
			/* Over 55 degeree - warning popuo and no charging */
			di->batt_thrm_state = BATT_THRM_WARNING_AND_STOP_CHARGING;
		} else if ( di->temp_C > batt_therm_data.with_pwr_current_limit_temp ) {
			/* 45 ~ 55 degree */
			if( di->voltage_uV > batt_therm_data.with_pwr_stop_charging_volt ) {
				/* Over 4.0V  - just stop charging */
				di->batt_thrm_state = BATT_THRM_STOP_CHARGING;
			} else {
				/* Under 4.0V - current limit */
				di->batt_thrm_state = BATT_THRM_CURRENT_LIMIT;
			}
		} else if ( di->temp_C > batt_therm_data.with_pwr_to_normal_temp ) {
			/* 42 ~ 45 degree */
			// Noting to do
			// if normal --> overr 42 degree... 
			//       remain old status ( BATT_THRM_NORMAL )
			// if over 45 degree --> under 45.... 
			//       remain old status ( BATT_THRM_STOP_CHARGING or BATT_THRM_CURRENT_LIMIT )
			;
		} else if ( di->temp_C > batt_therm_data.with_pwr_to_normal_low_temp ) {
			/* -5 ~ 42 degree */
			di->batt_thrm_state = BATT_THRM_NORMAL;
		} else if ( di->temp_C > batt_therm_data.with_pwr_stop_charging_low_temp) {
			/* -10 ~ -5 degree */
			// Nothing to do
			// if Normal --> under -5 degree...
			//      remain old status ( NORMAL )
			// if under -10 --> over -5
			//      remain old status ( BATT_THRM_STOP_CHARGING )
			;
		} else {
			/* Under -10 degree */
			di->batt_thrm_state = BATT_THRM_STOP_CHARGING;
		...
		/* Battery Only */
		if( di->temp_C > batt_therm_data.batt_only_pwroff_temp ) {
			/* 60 degree - power down */
			di->batt_thrm_state = BATT_THRM_PWROFF;
		} else if( di->temp_C > batt_therm_data.batt_only_warning_temp ) {
			/* 57 degree - warning popup */
			di->batt_thrm_state = BATT_THRM_WARNING_BATT_ONLY;
		} else {
			di->batt_thrm_state = BATT_THRM_NORMAL;
		}
	}
...
extern void set_charging_current_sub(void);
static void batt_thrm_current_limit(int decrease)
{
#if 0
#if defined(CONFIG_LGE_OMAP3_EXT_PWR)
...
#else
	if( decrease && ( TA_CABLE == get_ext_pwr_type() ) ) {
		/* Set Imaxsel 400mA */
		charging_ic_set_usb_mode();
	} else {
		/* Set Imaxsel to each case */
...
int need_to_batt_therm_sett = 0;
static int battery_too_hot_or_too_cold(struct twl4030_bci_device_info *di)
{
	int ret_value = 0;
	static int old_batt_therm_state = BATT_THRM_NORMAL;
	int cur_batt_therm_state = BATT_THRM_NORMAL;

	/* Current Setting Case */
	cur_batt_therm_state = di->batt_thrm_state;
	if( ( old_batt_therm_state != cur_batt_therm_state )
		|| ( need_to_batt_therm_sett ) )
	{
		switch(cur_batt_therm_state) {
			case BATT_THRM_NORMAL:
				batt_thrm_current_limit(0);
#ifdef CONFIG_LGE_CHARGE_CONTROL_BATTERY_FET
				lge_battery_fet_onoff(1);
#endif
				break;
			case BATT_THRM_CURRENT_LIMIT:
				batt_thrm_current_limit(1);
#ifdef CONFIG_LGE_CHARGE_CONTROL_BATTERY_FET
				lge_battery_fet_onoff(1);
#endif
				break;
			case BATT_THRM_STOP_CHARGING:
			case BATT_THRM_WARNING_AND_STOP_CHARGING:
				ret_value = 1;
				break;

			case BATT_THRM_WARNING_BATT_ONLY:
			case BATT_THRM_PWROFF:
			default :
				break;
		}
		printk(KERN_INFO "%s: *********** Batt Thrm State (%s)\n", __func__, batt_thrm_status_text[cur_batt_therm_state]);
		if( need_to_batt_therm_sett )
			need_to_batt_therm_sett = 0;
	}
	old_batt_therm_state = cur_batt_therm_state;
...
int get_block_charging_state(void)
{
	return block_charging_state;
}
static void batt_block_charging_set(int block)
{
	block_charging_state = block;

	switch(block_charging_state) {
		case 0: /* Block Charging */
		...
		case 1: /* Charging */
			...
			break;
		default :
			printk(KERN_INFO "%s: Invalid Value\n", __func__);
			break;
	}
	return ;
}
static unsigned int block_charging_time = 0;
static struct timeval block_chargig_time_start_time;
static void batt_block_time_out_set(int time_out_sec)
{
	/* if time_out_sec == -1 , it means Disable Chariging with no time out */
	if ( time_out_sec == -1 ) {
		printk("%s: ===================== Block Charging, No Time Out\n", __func__);
		batt_block_charging_set(0);
	} else {
		block_charging_time = time_out_sec;
		if( block_charging_time > 65535 )
			block_charging_time = 65535;

		if( block_charging_time ) {
			batt_block_charging_set(0);
			do_gettimeofday(&block_chargig_time_start_time);
		} else {
			batt_block_charging_set(1);
			memset((void*)&block_chargig_time_start_time, 0, sizeof(block_chargig_time_start_time) );
		}
	}
...
static int time_out_check(struct timeval start_time, unsigned int time_out_sec)
{
	static struct timeval current_time;
	int time_passed;

	do_gettimeofday(&current_time);
	if(start_time.tv_sec > current_time.tv_sec) {
		time_passed = start_time.tv_sec - current_time.tv_sec;
		time_passed = TIME_T_MAX - time_passed;
	} else {
...
static void valid_check_charging_block_time(void)
{
	if( block_charging_time ) {
		if( time_out_check(block_chargig_time_start_time, block_charging_time ) ) {
			batt_block_charging_set(1);
			memset((void*)&block_chargig_time_start_time, 0, sizeof(block_chargig_time_start_time) );
			block_charging_time = 0;
		} else {
			;
		}
	}
...
#error
	...
	if ( 0 == get_block_charging_state() ) {
		valid_check_charging_block_time();
		if ( 0 == get_block_charging_state() ) {
			di->charge_rsoc = POWER_SUPPLY_TYPE_BATTERY;
		}
	}
	...
	// Set Charging Status
/* LGE_CHANGE_S [LS855:bking.moon@lge.com] 2011-02-25, */ 
#if 0 /* mbk_temp */ 
	if(di->charge_rsoc == POWER_SUPPLY_TYPE_UPS ||		// JIG
	   di->charge_rsoc == POWER_SUPPLY_TYPE_BATTERY) {	// Battery
#else 
	if( di->charge_rsoc == POWER_SUPPLY_TYPE_BATTERY) {	// Battery
#endif 
/* LGE_CHANGE_E [LS855:bking.moon@lge.com] 2011-02-25 */
		di->charge_status = POWER_SUPPLY_STATUS_DISCHARGING;
	} else {
		if(di->battery_present == 0) {			// Battery not present. Display as not charging
			di->charge_status = POWER_SUPPLY_STATUS_NOT_CHARGING;		
#ifdef CONFIG_LGE_BATT_THERM_LAB3_SCENARIO
		} else if(di->batt_thrm_state == BATT_THRM_WARNING_AND_STOP_CHARGING ) {
			di->charge_status = POWER_SUPPLY_STATUS_NOT_CHARGING;
#else
...
#endif 
		} else if(chg_complete_state /* || end_of_charge */) {		// Charging FULL
...
typedef enum _charging_status_type {
	DISCHARGING_STATE,
	CHARGING_STATE,
	CHARGING_NEED_TIME_CHECK_STATE,
	CHARGING_COMPLETE_STATE,
	RECHARGING_STATE
} charging_status_type;

static char *charging_status_text[] = {
	"Discharging", "Charging", "Charging_time_check", "complete", "recharging"
};
...
static int set_battery_charging(struct twl4030_bci_device_info *di)
{
	...
	charging_status_type cur_charging_status = DISCHARGING_STATE;
	static charging_status_type old_charging_status = DISCHARGING_STATE;
	static int power_off_charging_cnt = 0;
	static int force_charging_done_cnt = 0;
	int complete_eoc_voltage = EOC_VOLTAGE_LIMIT;
#if 0
	...
	if(di->battery_present == 0 || 						// No Battery State
		...
		set_end_of_charge(0);
		set_charging_timer(0);
		old_charging_status = cur_charging_status = DISCHARGING_STATE;
		force_charging_done_cnt = 0;
		return 0;
	}

#ifdef CONFIG_LGE_BATT_THERM_LAB3_SCENARIO
	if( battery_too_hot_or_too_cold(di) )
#else
	if(di->temp_C < TEMP_CRITICAL_LOWER ||
	   di->temp_C > TEMP_CRITICAL_UPPER) 
#endif
	{		// Critical Temperature! Must stop charging
		chg_complete_state = 0;
		set_end_of_charge(0);
		set_charging_timer(0);
		if (di->battery_present == 1) {
	...
	if( lcd_off_boot )
		complete_eoc_voltage = 4200;

	/* Determine charging status */
	cur_charging_status = CHARGING_STATE;
	if(di->voltage_uV >= trickle_chg_max /* || charging_timeout() */ 
			|| ( end_of_charge && (di->voltage_uV >= complete_eoc_voltage)) ) {
		cur_charging_status = CHARGING_COMPLETE_STATE;
	} 

#if 1 /* Temp Code, Sometimes EOC intterupt is not occrued eventhough 4200V  */
	if( (!lcd_off_boot ) && (di->battery_capacity == 100) ) {

		if( end_of_charge == 0 ) {
			if( force_charging_done_cnt > 60 ) {
				cur_charging_status = CHARGING_COMPLETE_STATE;
			} else {
				//printk("[charging_msg] %s: need more charging case...\n");
				force_charging_done_cnt++;
			}
		}
	}
#endif
	...
	if( power_off_charging_state ) {
		/* need more charge - 3min */
		if( cur_charging_status == CHARGING_COMPLETE_STATE ) {

			if( power_off_charging_cnt > 100 ) {
				cur_charging_status = CHARGING_COMPLETE_STATE;
			} else {
				printk(KERN_INFO "[charging_msg] %s: Power off Charging.. Wait(%d/60) cnt... :(%s), Voltage %d, capacity %d, Endof Charging %d\n"
						, __func__, power_off_charging_cnt, charging_status_text[cur_charging_status]
						, di->voltage_uV, di->battery_capacity , end_of_charge);

				power_off_charging_cnt++;
				cur_charging_status = CHARGING_STATE;
			}
		}
	}
...
#if 0
	if( end_of_charge && (di->voltage_uV < complete_eoc_voltage) ) {
		printk("[charging_msg] %s: **************** EOC occured too early...\n", __func__);
	}
#endif

	if( (CHARGING_COMPLETE_STATE == old_charging_status) || 
	...
	if( cur_charging_status != old_charging_status ) {
		printk(KERN_INFO "[charging_msg] %s: Charging Status :(%s), Voltage %d, capacity %d, Endof Charging %d\n"
				, __func__, charging_status_text[cur_charging_status]
				, di->voltage_uV, di->battery_capacity , end_of_charge);
		...
		switch(cur_charging_status) {
			case CHARGING_STATE:
				...
				break;
#if 0
			case CHARGING_NEED_TIME_CHECK_STATE:
				set_charging_timer(1);
				break;
#endif
			case CHARGING_COMPLETE_STATE :
#ifdef CONFIG_LGE_CHARGE_CONTROL_BATTERY_FET
				lge_battery_fet_onoff(0);
#else 
				...
				set_charging_timer(0);
				set_end_of_charge(0);
				chg_complete_state = 1;
				break;
			case RECHARGING_STATE:
#ifdef CONFIG_LGE_CHARGE_CONTROL_BATTERY_FET
				lge_battery_fet_onoff(1);
#else 
				switch(di->charge_rsoc) {
					case POWER_SUPPLY_TYPE_MAINS:
#ifdef CONFIG_LGE_BATT_THERM_LAB3_SCENARIO
						if ( di->batt_thrm_state == BATT_THRM_CURRENT_LIMIT )
							charging_ic_set_usb_mode();
						else
#endif
						charging_ic_set_ta_mode();
		...
		old_charging_status = cur_charging_status;
	}
#endif
					    ...
					    const union power_supply_propval *val)
{
	...
	switch (psp) {
		...
		case POWER_SUPPLY_PROP_BATTERY_THRM_STATE:
			if( di->pseudo_batt_info.mode ) {
				batt_thrm_state_fake_mode = 1;
				di->batt_thrm_state = val->intval;
			} else {
				printk(KERN_ERR "%s: Need to set pseudo batt set\n", __func__);
			}
			break;
#endif
#ifdef CONFIG_LGE_BLOCK_CHARGING_MODE
		case POWER_SUPPLY_PROP_BLOCK_CHARGING:
			batt_block_time_out_set(val->intval);
			break;
#endif 
		default:
			return -EINVAL;
...
#ifdef CONFIG_LGE_BATT_THERM_LAB3_SCENARIO
		case POWER_SUPPLY_PROP_BATTERY_THRM_STATE:
			val->intval = di->batt_thrm_state;
			break;
#endif
#ifdef CONFIG_LGE_BLOCK_CHARGING_MODE
		case POWER_SUPPLY_PROP_BLOCK_CHARGING:
			val->intval = block_charging_time;
			break;
#endif 
		default:
			return -EINVAL;
...
#elif defined(CONFIG_LGE_OMAP3_EXT_PWR)
			if( (TA_CABLE == get_ext_pwr_type())
					|| (LT_CABLE_56K == get_ext_pwr_type())
					|| (LT_CABLE_130K == get_ext_pwr_type())
					|| (LT_CABLE_910K == get_ext_pwr_type()) ) 
			{
				val->intval = 1;
			} else {
				val->intval = 0;
